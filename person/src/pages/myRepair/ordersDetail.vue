...
export default {
   ...
   mounted() {
       ...
       vm.common.initWechat(['previewImage']);
       vm.querys();
   },
   methods: {
       querys() {
       ...
       showCommentPhoto(i) {
                        ...
                        vm.receiveData.getData(vm,"repair/finish/"+vm.item.id,'res',function(){
                           if(vm.res.success) { 
                            ...
                            //  vm.$router.push({path:'/repairPay',query:{ordersID:vm.item.id}})
                              location.href="https://www.e-shequ.com/qibao/weixin/orderpays.html?state=123#/repairPay?orderId="+vm.item.id+'&status=123'
                           }else {
                             alert("系统异常，请稍后重试！");
                           }  
                        },function() {
        ...
        comment() {
            ...
            // vm.$router.push({path:'/commentxiu',query:{orderId:vm.item.id}})
             location.href="https://www.e-shequ.com/qibao/weixin/orderpays.html?state=123#/commentxiu?ordersID="+vm.item.id
        },
        //返回
        back() {
             vm.$router.push({path:'/myrepair'})
