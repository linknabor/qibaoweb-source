   ...
   <div>
        <div class="adorder">
            <div class="order-number fs13 plr15">订单编号&nbsp;{{order.orderNo}}</div>
                <div class="p15">
                    <img class="fl order-picture" src="" />
                ...
                if(vm.res.success) {
                    ...
                    if(vm.order == null || vm.order.id == null){
                         MessageBox.confirm('获取订单补充信息信息失败，返回服务首页！').then(action => {
                             location.href="https://www.e-shequ.com/qibao/weixin/home/index.html?v=20160229";
                         }).catch(err => {
                            if(err == 'cancel') {
                  ...
                  if(vm.n.success) {
                      wx.chooseWXPay({
                        ...
                        "paySign":vm.n.result.signature,
                         ...
                         alert(vm.n.message==''?"支付请求失败，请稍后重试！":vm.n.message) 
                    }
            })
       },
         //通知
           ...
           MessageBox.confirm('确定要取消该服务？').then(action => {
                  if(action == 'confirm') {
                        vm.receiveData.getData(vm,"/cancelOrder/"+order.serviceOrderId,'n',function() {
                                            ...
                                            alert(vm.n.message==''?"支付取消失败，请稍后重试！":vm.n.message)  
                                            }
                            }) 
                     }
              }).catch(err => {
